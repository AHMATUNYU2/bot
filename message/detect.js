...
buttons = [
  {buttonId: 'p', buttonText: {displayText: 'Welcome semoga betah kak👋'}, type: 1}
]
const buttonsMessage = {
    imageMessage: msg,
    contentText: `${caption}`.trim(),    
footerText:`VISBOT By AHMATUNYU2 | © ${new Date().getFullYear()}`,
    buttons: buttons,
    headerType: "IMAGE"
}
...
msg = await caliph.toMSG(await buffer(canvas), 'imageMessage')
buttons = [
  {buttonId: 'h', buttonText: {displayText: 'bye BEBAN GRUB SELAMAT TINGGAL SAMPAI JUMPA LAGI 👋'}, type: 1}
]
buttonsMessage = {
    imageMessage: msg,
    contentText: `${caption}`.trim(),    
